...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === "--month") {
      ...
      args.posts = Number(argv[index + 1]);
      index += 1;
  ...
  if (args.posts !== null && (!Number.isInteger(args.posts) || args.posts < 1 || args.posts > 8)) {
    throw new Error("`--posts` must be an integer from 1 to 8.");
  }
...
function printHelp() {
  process.stdout.write(
    [
      "Usage:",
      "  node scripts/plan-month.js --month 2026-03",
      "  node scripts/plan-month.js --month 2026-03 --posts 4 --candidate-count 12",
      "  node scripts/plan-month.js --month 2026-03 --dry-run",
      "",
      "Behavior:",
      ...
      "  - Writes planning context and a Codex prompt to plans/YYYY-MM",
      "  - The shell runner uses Codex CLI to create plans/YYYY-MM/plan.json from that context",
      "",
    ].join("\n"),
  );
}
...
function buildMonthSlots(month, requestedPosts = null) {
  ...
  if (slots.length === 0) {
    slots.push(new Date(Date.UTC(year, monthIndex, 1)));
  }

  let chosen = slots;
  if (requestedPosts !== null && requestedPosts < slots.length) {
    const picked = [];
    for (let index = 0; index < requestedPosts; index += 1) {
      const position = Math.round((index * (slots.length - 1)) / Math.max(1, requestedPosts - 1));
      picked.push(slots[position]);
    }
    chosen = picked;
  } else if (requestedPosts !== null && requestedPosts > slots.length) {
    chosen = [];
    const start = new Date(Date.UTC(year, monthIndex, 1));
    const end = new Date(Date.UTC(year, monthIndex + 1, 0));
    for (let index = 0; index < requestedPosts; index += 1) {
      const offset = Math.round((index * (end.getUTCDate() - 1)) / Math.max(1, requestedPosts - 1));
      chosen.push(new Date(Date.UTC(year, monthIndex, start.getUTCDate() + offset)));
    }
  }

  return chosen.map((date, index) => ({
...
function loadRecentHistory(currentMonth) {
  ...
  return fs
    ...
    .filter((entry) => entry.isDirectory() && entry.name < currentMonth)
...
function buildCandidateShortlist(options) {
  ...
  const recentHistory = loadRecentHistory(month);
...
function buildPromptPayload(context) {
  ...
  const history = context.recent_history.length === 0
    ? "No prior monthly plans were found."
    : context.recent_history
        .map((plan) => {
          const posts = plan.posts
            .map((post) => `${post.slot}: ${post.content_key} (${post.screenshot_file})`)
            .join("; ");
          return `${plan.month}: ${posts}`;
        })
        .join("\n");

  return {
    system: [
      "You are planning monthly social posts for Hush Line.",
    ...
    user: [
      ...
      "",
      `Target dark-mode share for this month: ${context.dark_ratio}`,
      ...
      "- Avoid repeating the same content theme or route within the month.",
...
function buildCodexPrompt(context, planPath) {
  ...
  return [
    ...
    candidates,
    "",
    `Read planning context from: ${path.join("plans", context.month, "context.json")}`,
    ...
    "- Do not duplicate content_key values within the month.",
    "- Do not duplicate concept_key values within the month.",
...
function validatePlan(modelPlan, context) {
  ...
  if (modelPlan.month !== context.month) {
    throw new Error(`Model returned month ${modelPlan.month}, expected ${context.month}.`);
  }

  if (!Array.isArray(modelPlan.posts) || modelPlan.posts.length !== context.slots.length) {
    throw new Error("Model returned the wrong number of posts for the month.");
  }
  ...
  const posts = modelPlan.posts.map((post) => {
    ...
    if (usedContentKeys.has(post.content_key)) {
      throw new Error(`Model repeated content key ${post.content_key} within the month.`);
    }
    usedContentKeys.add(post.content_key);

    if (usedConceptKeys.has(candidate.concept_key)) {
      throw new Error(`Model repeated concept key ${candidate.concept_key} within the month.`);
    }
  ...
  return {
    ...
    posts: posts.sort((left, right) => left.slot.localeCompare(right.slot)),
    summary: modelPlan.summary,
  };
}

function buildPlanningContext(args) {
  const slots = buildMonthSlots(args.month, args.posts);
...
function writeContextArtifacts(month, context) {
  const planRoot = path.join(REPO_ROOT, "plans", month);
  ...
  fs.writeFileSync(promptPath, `${buildCodexPrompt(context, path.join("plans", month, "plan.json"))}\n`);
  return {
    contextPath,
    planPath,
    planRoot,
...
async function planMonth(args) {
  const context = buildPlanningContext(args);
  const artifacts = writeContextArtifacts(args.month, context);
