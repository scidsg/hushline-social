  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        cat <<'EOF'
Usage:
  ./scripts/agent_monthly_social_planner.sh --month 2026-03
  ./scripts/agent_monthly_social_planner.sh --month 2026-03 --posts 4 --candidate-count 12

Behavior:
  - Prepares monthly context and prompt artifacts under plans/YYYY-MM
  - Invokes Codex CLI in the local repo, following the daily runner pattern
  - Expects Codex to write plans/YYYY-MM/plan.json
  - Validates the plan and optionally renders assets into months/YYYY-MM
EOF
        exit 0
        ;;
      *)
...
build_context() {
  local -a cmd=(node scripts/plan-month.js --month "$MONTH" --dry-run)
  [[ -n "$POSTS" ]] && cmd+=(--posts "$POSTS")
...
reset_month_plan_artifacts() {
  rm -f "$REPO_DIR/plans/$MONTH/plan.json"
}
...
validate_and_render() {
  local -a cmd=(node scripts/validate-month-plan.js --month "$MONTH")
  [[ -n "$POSTS" ]] && cmd+=(--posts "$POSTS")
  [[ -n "$CANDIDATE_COUNT" ]] && cmd+=(--candidate-count "$CANDIDATE_COUNT")
  [[ -n "$DARK_RATIO" ]] && cmd+=(--dark-ratio "$DARK_RATIO")
...
main() {
  ...
  cp "$REPO_DIR/plans/$MONTH/prompt.txt" "$PROMPT_FILE"
  run_codex_from_prompt
  validate_and_render
}
