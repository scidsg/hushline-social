  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        cat <<'EOF'
Usage:
  ./scripts/agent_weekly_social_planner.sh --week 2026-W12
  ./scripts/agent_weekly_social_planner.sh --week 2026-W12 --candidate-count 12

Behavior:
  - Prepares weekly context and prompt artifacts under plans/YYYY-Www
  - Invokes Codex CLI in the local repo, following the daily runner pattern
  - Expects Codex to write plans/YYYY-Www/plan.json
  - Builds 5 weekday posts per weekly plan
  - Validates the plan and optionally renders assets into weeks/YYYY-Www
EOF
        exit 0
        ;;
      *)
...
build_context() {
  local -a cmd=(node scripts/plan-week.js --week "$WEEK" --dry-run)
...
reset_week_plan_artifacts() {
  rm -f "$REPO_DIR/plans/$WEEK/plan.json"
}
...
validate_and_render() {
  local -a cmd=(node scripts/validate-week-plan.js --week "$WEEK")
  [[ -n "$CANDIDATE_COUNT" ]] && cmd+=(--candidate-count "$CANDIDATE_COUNT")
  [[ -n "$DARK_RATIO" ]] && cmd+=(--dark-ratio "$DARK_RATIO")
...
main() {
  ...
  cp "$REPO_DIR/plans/$WEEK/prompt.txt" "$PROMPT_FILE"
  run_codex_from_prompt
  validate_and_render
}
